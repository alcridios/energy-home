...
def save_client(client, request):
    ...
    print('contact_register-> save_client')
    message_response = ''
    ...
    try:
        url = 'http://127.0.0.1:8001/api/v1/add-client/'
        print('CALL DELETE REST SERVICE -> {0}'.format(url))
        response = requests.post(url, data=json_client)
        print('response.status_code: {0}'.format(response.status_code))
        print(response.json())

        message_response = response.json()['message']
        client_j = response.json()['client']
        
        client = Client(id=client_j['id'],
                        rut=client_j['rut'],
                        nombre= client_j['nombre'],
                        direccion=client_j['direccion'],
                        email=client_j['email'],
                        telefono=client_j['telelfono'])
